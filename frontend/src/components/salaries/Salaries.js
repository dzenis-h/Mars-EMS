...
class Salaries extends Component {
    ...
    generateSalaries = () => {
        ...
        GenerateSalariesApi.GetDataFromAccountantSpreadsheet(months[this.state.month].label).then(response => {
            let employeeDataFromAccountant = response.data;
            GenerateSalariesApi.GetDataFromMainSpreadsheet().then(res => {
                employeeDataFromAccountant.map(item => {
                    for (let i = 0; i < res.data.length; i++) { // merging data from accountant and CEO's doc
                    if (item.jmbg === res.data[i].jmbg) {
                            ...
                            item.totalGrossSalary = item.totalNetSalary + item.bankContributes;
                            item.handSalary = (item.totalNetSalary - item.bankNetSallary - item.bankHotMeal);
                            item.handBonus = 0;
                            item.handPenalty = 0;
                ...
                console.log('error while getting data from accountant spreadsheet', error);
                throw(error);
            })
        });
    }
    ...
    getLastRowIndex = () => {
        return (Math.max(...this.props.employeesSalaries.map(elt => elt.lastRowNumber)) + 1);
    }

    goToNextStep = () => {
        const index = this.state.actionButtonStates.indexOf(this.state.currentButtonState);
        this.setState({
        ...
        if (this.state.currentButtonState === this.state.actionButtonStates[2]) {
            GenerateSalariesApi.ImportDataIntoSpreadsheet(this.state.year, this.state.months[this.state.month].label, this.getLastRowIndex(), this.props.employeesSalaries).then(res => {
                ClearDataApi.remove(this.state.month, this.state.year).then(res => {
        ...
        return (
            <div>
                <div className="container">
                    {(!this.state.shouldDisplayEmployeesDataTable && !this.state.shouldDisplayContentTabs && !this.state.shouldDisplayTotalPreview) &&
                        ...
                        <div className="portlet-body">
                            <div className="row">
                                        ...
                                        <div className="row">
                                            ...
                                            <div className="col-md-2 generate-salary__wrapper">
                                                <button type="button" className="btn btn-primary submit-button"
                                                        disabled={this.state.disableGenerateSalaryButton || !Number.isInteger(this.state.month) || !this.state.year}
                                                        onClick={this.generateSalaries}>Generate salary
                                                </button>
                                            </div>
                                        </div>
                                    </form>
                                ...
                                <div className="col-md-3 generate-salary-illustration_wrapper">
                                    <img src={generateSalaryIl} alt="Generate Salaty Illustration"
                    ...
                    <div className="portlet portlet-boxed">
                        <div className="portlet-header tab-content-portlet-header portlet-salaries">
                            ...
                            <button type="button" className="btn btn-primary btn-salaries"
                                    onClick={this.goToNextStep}>
                                {this.state.currentButtonState} <ArrowRightCircle size="18"/>
                            </button>
                            }
                        </div>
                    </div>}
                ...
                <div>
                    ...
                    <SalariesTable employeesSalaries={this.props.employeesSalaries}
                                   getEmployeesSalaries={this.props.getEmployeesSalaries}
                    />
                    }
                    {this.state.shouldDisplayContentTabs &&
                    <SalariesTabs year={parseInt(this.state.year, 10)} month={this.state.month}
                                  employeesSalaries={this.props.employeesSalaries}
                                  addEmployeesSalaries={this.props.addEmployeesSalaries}
                                  updateEmployeesSalaryRaises={this.props.updateEmployeesSalaryRaises}
                                  ...
                                  updateEmployeesLoanExtraPayments={this.props.updateEmployeesLoanExtraPayments}/>
                    }
                    {this.state.shouldDisplayTotalPreview &&
                    <SalariesTotalPreview employeesSalaries={this.props.employeesSalaries}/>
                    }
                </div>
