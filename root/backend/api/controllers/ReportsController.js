    ...
    getReports: (req, res) => {
        authentication.authenticate().then((auth) => {
          ...
          sheets.spreadsheets.values.get({
            ...
            const allEmps = reportsService.justNmes(updatedData);
        ...
        setTimeout(() => { 
            ...
            let theYears = longReduce(uniqueYears);      // returning unique and relevant months 
            ...
            const { length } = netSumArr[0];
            const activeEmps = Array.from(longestSalary, (_, i) => netSumArr.reduce((a, arr) => a + Boolean(arr[i]), 0));
...
longReduce = (longest) => { 
    let val = longest.reduce(function(a,i,ii){
    if (ii === 1){return a};
    if (i.length > a.length){return i}
    return a
  });
  return val;
}
