...
export default class SalariesTabs extends Component {
    ...
    deleteItem = (id) => {
        ContentApi.delete(id, this.state.activeTab.url).then(response => {
            setTimeout(() => {
                this.getByIdActiveTabContent(this.state.activeTab.url, this.props.year, this.props.month);
            }, 100)
        }).catch(error => {
    ...
    getByIdActiveTabContent = (year, month) => {
        SalaryContentApi.get(this.state.activeTab.url, year, month).then(response => {
            this.setState({
        ...
        const displayTabContent = this.state.displayData.map(item => {
            return <tr key={item.date.toString()}>
                ...
                <td className="col-md-1">
                    <a className="table-actions" onClick={this.openDeleteConfirmModal.bind(this, item.id)}><XCircle size="18"/></a>
                </td>
            </tr>
        });

        const tabs = this.state.tabs.map(item => {
            return <li className={`${item.active && 'active'}`} key={item.id}
