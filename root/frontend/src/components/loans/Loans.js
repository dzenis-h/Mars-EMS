...
class Loans extends Component {
    constructor(props) {
        ...
        saveBulkEmployees = (data) => {
            LoansApi.addBulk(data).then(response => {
                this.success();            
            ...
            let result = obj.map(a => { 
                ...
                this.setState((prevState) => ({
                    currentSession: {
                        ...
                        currentInstallment: [...prevState.currentSession.currentInstallment, a.lastInstallment],                        
                        loansData: [...prevState.currentSession.loansData, a.loansSum],
                        installmentsData: [...prevState.currentSession.installmentsData, a.installmentsSum],
                        remainingPaymentData: [...prevState.currentSession.remainingPaymentData, a.remainingPayment],
    ...
    const { peopleData, activeLoan, currentInstallment, loansData, installmentsData, remainingPaymentData, notesData } = this.state.currentSession;
    const { loanForm } = this.state;

    const employeesFormated = this.props.employees.filter(item=> item.enddate === undefined).map(emp => {
        return {
            "value": emp,
    ...
    const modalData = peopleData.map((item, idx) => {
            ...
            <div className="row">
                ...
                <div className="row">
                            ...
                            <form className="portlet-body">
                                ...
                                <div className="form-group">
                                    ...
                                    <input type="number" className="form-control" id="penaltyValue"
                                           ...
                                           name="amount"
                                           onChange={this.handleValueChange} value={loanForm.amount}/>
                                </div>
                                <div className="form-group">
                                    ...
                                    <input type="number" className="form-control" id="penaltyValue"
                                           ...
                                           name="installment"
                                           onChange={this.handleValueChange} value={loanForm.installment}/>
                                </div>
                                <div className="form-group">
                                ...
                                <div className="form-group">
                                    ...
                                    <textarea type="number" className="form-control" id="penaltyValue"
                                              placeholder="Enter Description" name="description"
                                              onChange={this.handleValueChange} value={loanForm.description}/>
                                </div>
                                <button type="button" style={{float: 'left'}} className="btn btn-primary" 
                                        onClick={this.addLoan}>Save loan(s)
                                </button>
                            </form>
                        </div>
                    </div>
                    <div className="col-md-8">
                        <div className="portlet portlet-boxed">
                            <div className="portlet-header">
                            <h4>Currently active loans <i className={`fa fa-long-arrow-right`}></i><span style={{ fontFamily: 'Arial' , color: '#3291b6', fontWeight: 'bold', letterSpacing: '3px', fontSize: '30px' }}
                            > {dataTable.length} </span></h4>
                                <p style= {{ color: '#48C6EF' }}>
                                Loans stats, monthly instalments, and notes <br/></p>
                            </div>
                        ...
                        <p style={{color:'#48C6EF', margin: '0px'}}> Loading ...</p>
                            ...
                            {!this.state.isLoading && this.state.currentSession.peopleData.length === 0 &&
                            <div className="portlet-body no-penalties__wrapper">
                                <img src={noPenaltiesIl} alt="Missing data illustration" className="no-data__image"/>
                                <p className="no-data">There are no active loans added at the moment! <br/>
                                You can add them by filling out te form on the left.</p>
                            </div>
                             }
                            {this.state.currentSession.peopleData.length > 0 &&
                <div className="portlet-body">

                <div><h6 style= {{color: '#48C6EF', float: 'right'}}>Loans summary &nbsp;&nbsp;
                    <PlusCircle style= {{ cursor: 'pointer'}} size="20" onClick={this.show} /></h6></div>
        ...
        <Rodal visible={this.state.visibleModal} onClose={this.hide} closeOnEsc={true} customStyles={{ height: 'auto', bottom: 'auto', top: '50%', transform: 'translateY(-50%)' }}>
                {<table className="table table-striped">
                    <thead><tr><th>Name</th><th>Total loan</th><th>Total Installment</th><th>Still Remaining</th>
                    </tr></thead><tbody>{modalData}</tbody></table>}
        </Rodal>
