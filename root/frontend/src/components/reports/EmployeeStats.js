...
class EmployeeStats extends Component {
    ...
    static contextTypes = {
        ...
        getEmployeeSalaryData = () => {
            ...
            return EmployeeApi.getEmployee(item)
                .then(employee => {
    ...
    render() { 
        ...
        let currentEmp = [];
        employees.map(y => {
            if (y.name + ' ' + y.surname == item) {
                return currentEmp.push(y);
            }}
        );

        // Getting start & end dates
        const empstartDate = currentEmp.map(x =>moment(x.startdate).format("M/YYYY"));        
        const empEndDate = currentEmp.map(x => {
            if (x.enddate !== undefined) {
                return moment(x.enddate).format("M/YYYY");
            } else {
                return <span style={{color: '#48c6ef'}}>Still active</span>;
            }
        });
        ...
        return (
            ...
            <div className="row navigation-row-2">
                <p className="btn btn-hollow" onClick={this.context.router.history.goBack}>
                <ArrowLeft size="18" className="button-left-icon"/> Go back to Details</p>
            </div>
       
            <div>
            <header style={{textAlign: 'center'}}>
            <h4>  <span style= {{ color: '#48C6EF', fontStyle: 'italic' }}> {onlyName}'s 
            </span> detailed payment info </h4>
            </header>
            
            <div className="col-md-6" style={{textAlign: 'center'}}>
            <p>START DATE <i className={`fa fa-long-arrow-right`}></i> 
                        <span style={{ fontFamily: 'Arial' , color: '#0ea55c', fontWeight: 'bold', letterSpacing: '2px', fontSize: '1,5rem' }}
                            > {empstartDate}</span></p>

            </div>
            <div className="col-md-6" style={{textAlign: 'center'}}>
            <p>END DATE <i className={`fa fa-long-arrow-right`}></i> 
                        <span style={{ fontFamily: 'Arial' , color: '#da002e', fontWeight: 'bold', letterSpacing: '2px', fontSize: '1,5rem' }}
                            > {empEndDate}</span></p>
            </div>
            <hr />   
        </div>

            <BootstrapTable keyField='rowNumber' 
                            striped
                            data={ finalForamt } 
                            columns={ columns } 
